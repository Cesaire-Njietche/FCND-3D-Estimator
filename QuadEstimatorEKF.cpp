...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  float r = rollEst, p = pitchEst;
  float v[9] = { 1, sin(r)*tan(p), cos(r)*tan(p), 0, cos(r), -sin(r), 0, sin(r)/cos(p), cos(r)/cos(p)};
  Mat3x3F R(v); // Matrix that turns intantaneous turn rate in body frame to inertial frame
 
  //Transform the angular velocity from body frame to inertial frame
  V3F i_rate = R* gyro;

  //Create a predicted attitude in the global frame
  float predictedPitch = pitchEst + dtIMU * i_rate.y ;
  float predictedRoll = rollEst + dtIMU * i_rate.x;
  ekfState(6) = ekfState(6) + dtIMU * i_rate.z;	// yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F accel_gf = attitude.Rotate_BtoI(accel * dt);
  ...
  predictedState(3) = predictedState(3) + accel_gf.x;
  predictedState(4) = predictedState(4) + accel_gf.y;
  predictedState(5) = predictedState(5) - 9.81f * dt + accel_gf.z;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  //Generating gPrime
  VectorXf ac_prime(3), ac(3);
  ac(0) = accel.x;
  ac(1) = accel.y;
  ac(2) = accel.z;
  ac_prime = RbgPrime * ac;
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = ac_prime(0) *dt;
  gPrime(4, 6) = ac_prime(1) *dt;
  gPrime(5, 6) = ac_prime(2) *dt;

  //Calculating the predicted covariance
...
void QuadEstimatorEKF::Update(VectorXf& z, MatrixXf& H, MatrixXf& R, VectorXf& zFromX)
{
  ...
  VectorXf zt(z.size());
  zt = z - zFromX;
  if (z.size() == 1)//If the update function is call whule uptading yaw, normalize the difference btw the measured and the estimate yaw
  {
	  if (zt(0) > F_PI) zt(0) -= 2.f*F_PI;
	  if (zt(0) < -F_PI) zt(0) += 2.f*F_PI;
  }
  ekfState = ekfState + K*(zt);
